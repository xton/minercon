...
export type LogLevelResolution = { level: number } | { error: string };
...
export function resolveLogLevel(flagValue: string | undefined, envValue: string | undefined): LogLevelResolution {
  ...
  if (raw in LogLevels) {
    return { level: LogLevels[raw as LogType] };
  }
  return { error: `invalid log level: ${raw} (expected one of: ${Object.keys(LogLevels).join(', ')})` };
}
